...
let erc1155Token, creator, other1, other2, event;
...
describe("Token1155 test", function (accounts) {
  ...
  it("token URI setup for ID", async function () {
    ...
    await erc1155Token.connect(creator).setTokenUri(5, baseURI + "5.json");
    await erc1155Token.connect(creator).setTokenUri(6, baseURI + "6.json");
    await erc1155Token.connect(creator).setTokenUri(7, baseURI + "7.json");
    await erc1155Token.connect(creator).setTokenUri(8, baseURI + "8.json");
    await erc1155Token.connect(creator).setTokenUri(9, baseURI + "9.json");
    await erc1155Token.connect(creator).setTokenUri(10, baseURI + "10.json");
  });

  describe("minting", function () {
    it("users can not mint tokens", async function () {
      await expect(
        erc1155Token.connect(other1).mint(other1.address, 1, 5, 0x11)
    ...
    it("creator can mint 5 token with id=1 for other account", async function () {
      tx = await erc1155Token
        .connect(creator)
        .mint(other1.address, 1, 5, 0x112233);
      // get event
    ...
    it("creator can mint 15 token with id=2 for other2 account", async function () {
      await expect(
        erc1155Token.connect(creator).mint(other2.address, 2, 15, 0x11)
      )
        .to.emit(erc1155Token, "TransferSingle")
        .withArgs(creator.address, AddressZero, other2.address, 2, 15);
    ...
    it("creator can batch mint some tokens with different ids for other1 account", async function () {
      await expect(
        erc1155Token
          .connect(creator)
          .mintBatch(other1.address, [3, 4], [8, 16], 0x11)
      )
      ...
      expect(await erc1155Token.balanceOf(other1.address, 4)).to.be.equal(
        BigNumber.from("16")
      );
      ...
      expect(await erc1155Token.totalSupply(4)).to.be.equal(
        BigNumber.from("21")
      );
  ...
  describe("transfer", function () {
    it("other2 can transfer their tokens", async function () {
      await erc1155Token
        .connect(other2)
        .safeTransferFrom(other2.address, other1.address, 2, 3, 0x11);
    ...
    it("other1 can batch transfer their tokens", async function () {
      ...
      expect(await erc1155Token.balanceOf(other2.address, 4)).to.be.equal(
        BigNumber.from("5")
      );
    });
  });
